...
let RECWIDTH = 320;
const RECHEIGHT = 32;
const canvas_det = new OffscreenCanvas(1, 1);
const canvas_rec = new OffscreenCanvas(1, 1);
...
function initCanvas(img: ImageBitmap) {
...
export async function recognize(
    ...
    const text_list = [];
    const imageCanvas = initCanvas(image)
    for (let i = 0; i < boxes.length; i++) {
        const tmp_box = JSON.parse(JSON.stringify(boxes[i]));
        get_rotate_crop_image(imageCanvas, tmp_box);
...
function get_rotate_crop_image(img: OffscreenCanvas, points: number[]) {
...
async function recognizeEvent({ image: imageBitmap, canvasOutput }: { image: ImageBitmap, canvasOutput: boolean }) {
    ...
    if (canvasOutput) {
        ...
        const image = await createImageBitmap(canvas.getContext('2d').getImageData(0, 0, canvas.width, canvas.height), 0, 0, canvas.width, canvas.height);
        webWorker.postMessage({
            event: 'recognize',
            data: {
                ...res,
        ...
        const res = await recognize(imageBitmap);
        webWorker.postMessage({
            event: 'recognize',
            data: {
                ...res,
...
webWorker['HTMLImageElement'] = class A { }
webWorker['HTMLCanvasElement'] = OffscreenCanvas
